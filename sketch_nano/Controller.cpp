...
const uint8_t BTN_INPUTS[] = {2, 3, 4, 5, 6, 7};
const uint8_t BTN_OUTPUT[] = {A1, A2, A3, A4};

Controller::Controller(/*int pinData*/) {
    blink=true;
    FastLED.addLeds<WS2812B, 8, GRB>(leds, 24);
    ...
    for(i=0 ; i < sizeof(BTN_OUTPUT) ; i++) {
        ...
        for(j=0 ; j < sizeof(BTN_INPUTS) ; j++) {
            if(digitalRead(BTN_INPUTS[j]) == HIGH) {
                const long point = millis();
                while(millis()-point < 300) ; //short-circuit filtering
                if(digitalRead(BTN_INPUTS[j]) == HIGH) {
